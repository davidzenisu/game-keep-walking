...
import 'package:game_keep_walking/util.dart';
...
class HealthAppState extends State<HealthApp> {
  ...
  Future<void> getSkinTemperatureFeatureStatus() async {
    if (!Platform.isAndroid) {
      setState(() {
        ...
        _state = AppState.skinTemperatureFeatureStatus;
      });
      return;
    }
  ...
  Future<void> createChangesToken() async {
    if (!Platform.isAndroid) {
      setState(() => _state = AppState.changesNotReady);
      return;
    }
    ...
    try {
      ...
      setState(() {
        ...
        _changes.clear();
        _state = token == null ? AppState.changesNotReady : AppState.changesReady;
      });
    } catch (error) {
  ...
  Future<void> writeSkinTemperatureSample() async {
    if (!Platform.isAndroid) {
      setState(() => _state = AppState.dataNotAdded);
      return;
    }
  ...
  List<WorkoutRouteLocation> _buildSampleWorkoutRoute(
    ...
    final intervalMillis = (clampedEnd.millisecondsSinceEpoch - start.millisecondsSinceEpoch) ~/
  ...
  Future<void> revokeAccess() async {
    ...
    setState(() {
      _state = success ? AppState.permissionsRevoked : AppState.permissionsNotRevoked;
    });
  }
      ...
      home: Scaffold(
        ...
        body: Column(
          children: [
            ...
            if (_state == AppState.dataReady) _dataFiltration,
            if (_state == AppState.stepsReady) _stepsFiltration,
  ...
  Widget get _contentChangesReady {
    final token = _changesToken;
    final tokenPreview =
        token == null ? 'none' : (token.length > 12 ? '${token.substring(0, 12)}...' : token);
  ...
  Widget get _content => switch (_state) {
        ...
        AppState.healthConnectStatus => _contentHealthConnectStatus,
        AppState.skinTemperatureFeatureStatus => _contentSkinTemperatureFeatureStatus,
        AppState.permissionsRevoking => _permissionsRevoking,
        AppState.permissionsRevoked => _permissionsRevoked,
        AppState.permissionsNotRevoked => _permissionsNotRevoked,
        AppState.changesReady => _contentChangesReady,
        AppState.changesNotReady => _contentChangesNotReady,
      };
